...
#' @examples
#' 
WaveletSS.xreg<- function(y,xreg,NForecast, order=1, freq=12, method=F,flag.comb="BG",...)
{
  n1 <- length(y)-NForecast
  train_o <- y[1:n1]
  test_o <- y[-c(1:n1)]
  ...
  if(method){
    ...
    WaveletFit <- dlmMLE(y=as.ts(train_o), parm=par, X=xreg[1:n1,], build=ssm) 
    if(WaveletFit$convergence==0) print("converge") else print("Did not converge")
    ...
    Accuracy.Train = accuracy(as.ts(WaveletPredict), train_o)
    Accuracy.Test =  accuracy(as.ts(WaveletForecast), test_o)
